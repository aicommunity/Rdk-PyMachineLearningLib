...
void TPythonIntegration::AInit(void)
{
    try
    {
        ...
        py::object PythonIntegrationInterface = RDK::import("python_integration_interface",
         ...
         MainNamespace);
        // экземпляр питоновского класса, через который можно выбрать virtualenv-среду
        py::object VirtualEnvInitializer = PythonIntegrationInterface.attr("VirtualEnvInitializer")();
        const char* TmpVirtualenv = std::getenv("VIRTUAL_ENV");  // проверяем, задан ли путь до virtualenv-серды
        ...
        if (TmpVirtualenv) {
            ...
            VirtualEnvInitializer.attr("activate_virtual_environment")(VirtualEnvPath);  // активация выбранной среды по пути
        }

        // инициализируем экземпляр класса, через который будем вызывать нужные питоновские методы
